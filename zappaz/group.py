...
async def list_groups(sessionId: str, token: str):
    ...
    response = await client.get(
        url=f"/session/{sessionId}/group", 
        headers=headers
    )

    if response.status_code != 200:
        return {"error": f"Failed to fetch groups: {response.status_code}"}

    content = response.json()
    
    res = {
        "totalGroups": len(content),
        "groups": [{**group,"totalParticipants": len(group.get("participants", []))} for group in content]
    }
...
async def remove_participants(data: PhoneList, groupId: str, sessionId: str, token: str):
    headers = get_headers(token)
    response = await client.post(
...
async def promote_participants(data: PhoneList, groupId: str, sessionId: str, token: str):
    headers = get_headers(token)
    response = await client.post(
...
async def demote_participants(data: PhoneList, groupId: str, sessionId: str, token: str):
    headers = get_headers(token)
    response = await client.post(
...
async def send_group_invite_link(data: InviteLink, groupId: str, sessionId: str, token: str):
    headers = get_headers(token)
    response = await client.post(
